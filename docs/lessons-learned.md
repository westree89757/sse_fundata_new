...
### 教训 16: 数据源切换时必须验证字段单位一致性

**时间**: 2026-05-12  
**问题**: 切换到 mootdx 后，成交量显示 100 倍偏差，阶段分析趋势变形，风险提示消失  
**根因**: mootdx `frequency=9` 返回的 `vol` 单位是**手**（每手 100 股），而 Sina 返回的是**股**。数据源切换时未做单位归一化，导致成交量数值差 100 倍。份额插值中，最新 SSE 采样点之后的所有日期共用同一个份额值，阶段无法判定。  
**修复**: `_mootdx_etf` 和 `_mootdx_index` 中 volume 乘以 100 转为股，手动刷新拉取最新份额数据  
**经验**:
- 每个新数据源接入后必须对比验证数值量级（取同一只 ETF、同一天的数据逐字段对比）
- 数据源返回的字段可能有不同含义（`vol` vs `volume`）、不同单位（手 vs 股）
- 切换数据源后应运行端到端验证：API → 前端显示 → 图表趋势是否合理
- 份额插值在最后采样点之后无"下一个"数据点时，只能沿用前值——这是已知限制，但不影响阶段分析

### 教训 17: auto_import 不应该覆盖已有精确数据

**时间**: 2026-05-12  
**问题**: `auto_import_from_json()` 用 `upsert_etf_basic` 覆盖了 ETF 名称、份额、溢价等精确数据，名称变成代码，溢价全部 N/A  
**根因**: GitHub Actions 生成的 JSON 不含 ETF 名称/溢价（这些字段仅 AKShare 的 `fund_etf_fund_daily_em` 提供），但 auto-import 用空值覆盖了 DB 中的完整数据  
**修复**: 移除 auto-import 中的 `upsert_etf_basic` 调用，仅在手动刷新（`fetcher.py`）中写入 ETF basic 信息  
**经验**:
- 不同数据入口（GitHub Actions vs 本地手动刷新）能获取的数据维度不同，导入时要区分"我能提供的数据"和"已有更精确的数据"
- `INSERT OR REPLACE` 是整行替换，不区分字段来源——空字段会覆盖已有的有效数据
- 宁可少写，不要覆盖
